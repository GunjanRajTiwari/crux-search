...
const createPhrases = (caption: string): Phrase[] => {
	...
	for (let i = 0; i < words.length; i++) {
		...
		if (
			(currentPhrase.split(/\s+/).length >= 3 &&
				currentPhrase.length > 15 &&
				i < words.length - 1 &&
				words[i].match(/[.,!?;:]$/)) ||
			i === words.length - 1 ||
			currentPhrase.split(/\s+/).length >= 6
		) {
...
const ReelViewer: React.FC<ReelViewerProps> = ({
	...
	useEffect(() => {
		if (slide && slide.caption && !isAdView) {
			phrasesRef.current = createPhrases(
				(slide as ReelSlide).caption
			);
		} else {
			phrasesRef.current = [];
		}
	...
	useEffect(() => {
		...
		if (isPlaying) {
			if (
				...
				utteranceRef.current.text === slide.caption
			) {
				...
				utterance.onboundary = event => {
					if (
						event.name === "word" &&
						phrasesRef.current &&
						phraseIdxForBoundary < phrasesRef.current.length
					) {
						const currentPhraseText =
							phrasesRef.current[phraseIdxForBoundary].text;
						if (
							event.charIndex >=
							charIndexAccumulator + currentPhraseText.length
						) {
							charIndexAccumulator +=
								currentPhraseText.length + 1;
							...
							event.charIndex >= charIndexAccumulator &&
							phraseIdxForBoundary !== currentPhraseIndex
						) {
	...
	if (!slide && !isLoadingImage) {
		return (
			<div
				className='reel-viewer-container flex items-center justify-center text-neutral-400'
				style={{
	...
	return (
		<div className='reel-viewer-container'>
			<div className='story-progress-bars'>
				{Array.from({
					length: totalSlides > 0 ? totalSlides : 1,
				}).map((_, index) => {
					let fillWidth = "0%";
					...
					return (
						<div
							key={index}
							className='story-progress-bar-segment'>
							<div
			...
			{!isAdView && slide && totalSlides > 0 && (
				<>
					<div
						className='reel-tap-zone reel-tap-zone-prev'
						onClick={onPrevClicked}
						role='button'
						aria-label='Previous slide'></div>
					<div
						className='reel-tap-zone reel-tap-zone-playpause'
					...
					<div
						className='reel-tap-zone reel-tap-zone-next'
			...
			<div className='w-full h-full absolute inset-0'>
				{slide?.imageUrl ? (
					...
					<div
						...
						{" "}
						{/* Keep reel BG dark for placeholder */}
						<i
							className={`fas ${
								isLoadingImage
									? "fa-spinner fa-spin"
							...
							style={{ color: "var(--reel-text-main)" }}></i>
						{isLoadingImage && (
							<p
								className='ml-3 text-sm'
								style={{ color: "var(--reel-text-main)" }}>
								Visualizing...
							</p>
						)}
					</div>
			...
			<div className='absolute inset-0 flex flex-col justify-between p-4 sm:p-5 bg-gradient-to-t from-black/80 via-black/30 to-transparent pointer-events-none'>
				<div className='flex justify-end items-start'>
					{isAdView && (
						<div
							className='text-xs font-semibold px-2.5 py-1 rounded-md shadow'
							style={{
								backgroundColor:
									"var(--reel-sponsored-bg)",
								color: "var(--reel-sponsored-text)",
							}}>
							Sponsored
						</div>
					)}
				...
				<div className='mb-4'>
					{isAdView && adSlideData ? (
						<div style={{ color: "var(--reel-text-main)" }}>
							<p
								className='font-semibold text-xl sm:text-2xl mb-1.5 leading-tight'
								style={{
									textShadow:
						...
						phrasesRef.current.length > 0 && (
							<div
								className='text-2xl sm:text-3xl leading-tight font-semibold'
								style={{ color: "var(--reel-text-main)" }}>
								{phrasesRef.current.map(
									(phrase, index) => (
										<span
											key={index}
											className={`caption-phrase ${
												index ===
												currentPhraseIndex
